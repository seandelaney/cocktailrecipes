...
class CocktailRecipes_IngredientsControllerTest extends PHPUnit_Framework_TestCase
{
    public function setUp()
    {
        ...
        $this->cocktailRecipes_ingredients = m::mock('Craft\CocktailRecipesService');
    ...
    public function testSaveIngedient()
    {
        ...
        $this->cocktailRecipes_ingredients->shouldReceive('getIngredientById')->with(5)->once()
            ->andReturn($mockModel);
        ...
        $this->cocktailRecipes_ingredients->shouldReceive('saveIngredient')->with($mockModel)->once()
            ->andReturn(true);
    ...
    public function testSaveIngredientNew()
    {
        ...
        $mockModel = m::mock('Craft\CocktailRecipes_IngredientModel');
        $this->cocktailRecipes_ingredients->shouldReceive('newIngredient')->once()
            ->andReturn($mockModel);
    ...
    public function testDeleteIngredient()
    {
        ...
        $this->request->shouldReceive('getRequiredPost')->with('id')->once()
            ->andReturn(5);
        $this->cocktailRecipes_ingredients->shouldReceive('deleteIngredientById')->with(5)->once();
